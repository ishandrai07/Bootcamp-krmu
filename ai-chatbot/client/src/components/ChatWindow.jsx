...
function ChatWindow({ onClose }) {
  const [messages, setMessages] = useState([
    { role: "assistant", text: "Hi! I'm your AI assistant. Ask me anything, by typing or by voice." },
  ]);
  ...
  // Builds the Mistral-style history array from our simple messages state.
  // Mistral requires history to start with role "user", so we drop any
  // leading "assistant" messages (like our initial greeting) before sending.
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", text: "Sorry, I couldn't reach the server. Is it running?" },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="chat-window">
      ...
      <div className="chat-messages">
        {messages.map((msg, i) => (
          <div key={i} className={`chat-bubble ${msg.role === "assistant" ? "model" : msg.role}`}>
            {msg.text}
          </div>
        ))}
